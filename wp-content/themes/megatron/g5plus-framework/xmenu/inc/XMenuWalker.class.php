...
class XMenuWalker extends Walker_Nav_Menu {
	...
	function add_custom_style ($args) {
		...
		if ( $menu && ! is_wp_error($menu) && !isset($menu_items) ) {
			...
			if ($menu_items) {
				...
				$responsive_breakpoint = isset($g5plus_options['mobile_header_responsive_breakpoint'])
	...
	function start_lvl( &$output, $depth = 0, $args = array() ) {
		$items_defaults = xmenu_get_item_defaults();
		$menu_parent = null;
		...
		if ($xmenu_meta) {
			...
			$xmenu_meta = array_merge($items_defaults, $xmenu_meta);
		}
		else {
			$xmenu_meta = $items_defaults;
		}
		...
		if ($menu_parent) {
			...
			if ($xmenu_parent_meta) {
				...
				$xmenu_parent_meta = array_merge($items_defaults, $xmenu_parent_meta);
			}
			else {
				$xmenu_parent_meta = $items_defaults;
			}
		}
