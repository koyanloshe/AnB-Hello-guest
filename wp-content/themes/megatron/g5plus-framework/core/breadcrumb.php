...
if (!function_exists('g5plus_get_breadcrumb_items')) {
	function g5plus_get_breadcrumb_items() {
		...
		/* Front page. */
		if (is_front_page()) {
			$item['last'] = esc_html__('Home', 'g5plus-megatron' );
		}


		/* Link to front page. */
		if (!is_front_page()) {
			$item[] = '<li><a href="' . home_url('/') . '" class="home">' . esc_html__('Home', 'g5plus-megatron' ) . '</a></li>';
		}
		...
		elseif (is_singular()) {
			...
			if (is_category() || is_tag() || is_tax()) {
				...
				if (is_day())
					$item['last'] = esc_html__('Archives for ', 'g5plus-megatron' ) . get_the_time('F j, Y');

				elseif (is_month())
					$item['last'] = esc_html__('Archives for ', 'g5plus-megatron' ) . single_month_title(' ', false);

				elseif (is_year())
					$item['last'] = esc_html__('Archives for ', 'g5plus-megatron' ) . get_the_time('Y');
			} else if (is_author())
				$item['last'] = esc_html__('Archives by: ', 'g5plus-megatron' ) . get_the_author_meta('display_name', $wp_query->post->post_author);

		} /* If viewing search results. */
		else if (is_search()) {
			//$item['last'] = esc_html__('Search results for "', 'g5plus-megatron' ) . stripslashes(strip_tags(get_search_query())) . '"';
			$item['last'] = esc_html__('Search results', 'g5plus-megatron');
		}


		/* If viewing a 404 error page. */
		else if (is_404())
			$item['last'] = esc_html__('Page Not Found', 'g5plus-megatron' );
