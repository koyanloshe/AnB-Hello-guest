...
function matthewruddy_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
		...
		if ( empty( $url ) )
			return new WP_Error( 'no_image_url', esc_html__( 'No image URL has been entered.','g5plus-megatron' ), $url );
		...
		$name = wp_basename( $file_path, ".$ext" );
	        ...
	        if ( 'bmp' == $ext ) {
			return new WP_Error( 'bmp_mime_type', esc_html__( 'Image is BMP. Please use either JPG or PNG.','g5plus-megatron' ), $url );
		}
