...
if (!function_exists('g5plus_enqueue_styles')) {
	function g5plus_enqueue_styles() {
		...
		/*owl-carousel*/
		wp_enqueue_style('g5plus_framework_owl_carousel', G5PLUS_THEME_URL . 'assets/plugins/owl-carousel/assets/owl.carousel'.$min_suffix.'.css', array());

		/*prettyPhoto*/
		wp_enqueue_style('g5plus_framework_prettyPhoto', G5PLUS_THEME_URL . 'assets/plugins/prettyPhoto/css/prettyPhoto'.$min_suffix.'.css', array());

		/*peffect_scrollbar*/
		wp_enqueue_style('g5plus_framework_peffect_scrollbar', G5PLUS_THEME_URL . 'assets/plugins/perfect-scrollbar/css/perfect-scrollbar.min.css', array());

		/*slick*/
		wp_enqueue_style('g5plus_framework_slick', G5PLUS_THEME_URL . 'assets/plugins/slick/css/slick'.$min_suffix.'.css', array());
...
if (!function_exists('g5plus_custom_font_styles')) {
    function g5plus_custom_font_styles()
    {
        $g5plus_options = &G5Plus_Global::get_options();
        $custom_font_css = '';
    ...
    add_action('wp_head', 'g5plus_custom_font_styles', 100);
    add_action('admin_head', 'g5plus_custom_font_styles', 100);
}
...
if (!function_exists('g5plus_enqueue_script')) {
	function g5plus_enqueue_script() {
		...
		if ( isset($g5plus_options['panel_selector']) && ($g5plus_options['panel_selector'] == 1)) {
			wp_enqueue_script('g5plus_framework_panel_selector', G5PLUS_THEME_URL . 'assets/js/panel-style-selector' . $min_suffix . '.js', array(), false, true);
		}

		wp_enqueue_script( 'g5plus-jplayer-js', G5PLUS_THEME_URL . 'assets/plugins/jquery.jPlayer/jquery.jplayer.min.js', array(), '', true );

		/*slick*/
		wp_enqueue_script('g5plus_framework_slick', G5PLUS_THEME_URL . 'assets/plugins/slick/js/slick' . $min_suffix . '.js', array(), false, true);

		wp_enqueue_script('g5plus_framework_app', G5PLUS_THEME_URL . 'assets/js/main' . $min_suffix . '.js', array(), false, true);

		// Localize the script with new data
		$translation_array = array(
			'product_compare' => esc_html__('Compare','g5plus-megatron'),
			'product_wishList' => esc_html__('WishList','g5plus-megatron')
		);
